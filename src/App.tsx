...
const nlTable = questions;
const frTable = Object.entries(questions).reduce<Record<string, string>>((table, [s, n]) => { table[n] = s; return table }, {});
...
type FrMode = typeof GameModes.fr;
type NlMode = typeof GameModes.nl;
type GameMode = FrMode | NlMode;
...
const storagePrefix = "20_06_2022"

function gameModeSerializer() {
  const storageKey = `${storagePrefix}_GAME_MODE`;
  const stored = localStorage.getItem(storageKey)
  const initial: GameMode = stored as GameMode || GameModes.fr;
...
function getRandomEntries(mode: GameMode) {
  ...
  const table = mode === GameModes.nl ? nlTable : frTable;
...
const initialRandomEntries = {
  [GameModes.fr]: getRandomEntries(GameModes.fr),
  [GameModes.nl]: getRandomEntries(GameModes.nl),
}
...
function App() {
  ...
  const onGuessNL = React.useCallback(() => {
    setMode(GameModes.nl)
    setCorrect(0);
    setWrong(0);
    setCurrentIndex(0);
  }, []);

  const onGuessFR = React.useCallback(() => {
    setMode(GameModes.fr)
    setCorrect(0);
    setWrong(0);
    setCurrentIndex(0);
  }, []);
  ...
  return (
    <div className="App">
      <div>
        <button onClick={onGuessNL} className={mode === GameModes.nl ? "active" : ""}>NL</button>
        &nbsp;
        &nbsp;
        &nbsp;
        <button onClick={onGuessFR} className={mode === GameModes.fr ? "active" : ""}>FR</button>
      </div>
      <br />
      <div>
        <span ref={correctRef}>
